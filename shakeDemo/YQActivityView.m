...
- (NSArray *)points{
    ...
    for (int i = 0; i < count+1; i ++) {
        ...
        //创建装N根线的数组
        for (int a = 0; a < self.linesCount; a++) {
            [tempArr addObject:[NSMutableArray array]];
        }
        
        
        for (int j = 0; j < self.linesCount; j ++) {
            
            //点增量半径
            CGFloat subRadius = radius + 8*j;
            ...
            if (pAngle<=90) {
                ...
                x = center.x + sin(aAngle) * subRadius;
                y = center.y - cos(aAngle) * subRadius;
            }else if (pAngle<=180){
                aAngle = degreesToRadians(pAngle-90);
                x = center.x + cos(aAngle) * subRadius;
                y = center.y + sin(aAngle) * subRadius;
            }else if (pAngle<=270){
            ...
            NSMutableArray *mutArr = tempArr[j];
            [mutArr addObject:(i==count)?[mutArr firstObject]:pointValue];
        }
...
- (void)drawRect:(CGRect)rect {
    ...
    for (NSMutableArray *pointArray in self.points) {
...
//    [bezPath closePath];
    ...
    contentLayer.strokeColor = [UIColor greenColor].CGColor;//[UIColor colorWithHexString:self.colorHexs[self.colorHexs.count-i-1]].CGColor;
    contentLayer.lineWidth = 1.3;
    contentLayer.path = bezPath.CGPath;
    
    [self.layer addSublayer:contentLayer];
    self.lineLayer1 = contentLayer;
